...
def ipa_chain_aggregator():
    df = pd.read_csv("../output/final/final/output_after_screaning_v4_ja_screaning.csv", sep=',', index_col=0)
    print(len(df[~df.duplicated()]))
    df_concat_d = df[~df.duplicated()]
    ...
    df_concat_d_r.to_csv(f'../output/final/final/output_after_screaning_v4_ja_screaning_next_search_index_screaning.csv')
    print(f"finish writing output to csv/ {len(df_concat_d_r)}")
    print(f"{match_full_pair_count} / {len(df_concat_d_r)}")
    if match_full_pair_count == len(df_concat_d_r):
        print("終了")
